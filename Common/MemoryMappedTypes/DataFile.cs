...
public readonly ref struct MapFeatureData
{
    ...
    public Dictionary<DrawableTerrainType, string> Properties { get; init; }
}
    ...
    public void ForeachFeature(BoundingBox b, MapFeatureDelegate? action)
    {
        ...
        for (var i = 0; i < tiles.Length; ++i)
        {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j)
            {
                ...
                if (isFeatureInBBox)
                {
                    var properties = new Dictionary<DrawableTerrainType, string>(feature->PropertyCount);
                    for (var p = 0; p < feature->PropertyCount; ++p)
                    {
                        ...
                        if (Enum.IsDefined(typeof(DrawableTerrainType), key.ToString())) //< We have to do a check if we have that type in our enum 
                        {
                            DrawableTerrainType AddKeys = (DrawableTerrainType)Enum.Parse(typeof(DrawableTerrainType), key.ToString());
                            properties.Add(AddKeys, value.ToString()); //< Adds the keys as strings to the properties dict
                        }
                    }
